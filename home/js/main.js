$(document).ready(function(){
	...
	//PC용 서브메뉴 사라지는 액션(아래)
	var isOver1 = false; //false 일때 서브메뉴가 사라지는 조건
	var isOverSub21 = false; //서브메뉴가1번이 flase 일때 서브메뉴가 사라지는 조건
	//1번 서브메뉴를 숨기는 함수(아래)
	function goHide1(){
		if(!isOver1 && !isOverSub21){//2개의 변수 값이 false일때 구현조건이 됩니다.
			$('.gnb_depth2_1').stop().fadeOut('fast');
		}
	}
	var isOver2 =false; //대메뉴2이 false 일때 서브메뉴가 사라지는 조건
	var isOverSub2 =false;//서브메뉴가2번이 flase 일때 서브메뉴가 사라지는 조건
	//2번 서브메뉴 숨기는 함수(아래)
	function goHide2(){
	...
	$(document).ready(function(){
		...
		$('.openAll1').mouseover(function(){
			if( parseInt($('header').css('width')) >=1049) {
				$('.gnb_depth2_1').stop().fadeIn('slow');//서브메뉴1번 나타나게 처리
			}
			isOver1 = true;// false일때 숨기는 조건 <-> true일때 나타나는 조건
		});
		$('.openAll1').mouseout(function(){
			isOver1 = false;
			setTimeout('goHide1()',200);// 기본은 goHide1() 이것만하면되는데, mouseout이벤트 놓치면 안되기때문에 setTimeout사용
		});
		// 서브메뉴1 액션
